...
#[tokio::test]
async fn test_hpack_sensitive_headers_transmit_successfully() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Pre-populate the cookie store to ensure the cookie header is sent.
    {
        ...
        store.parse("session_id=12345abcdef; Secure; HttpOnly", &target_url).unwrap();
    }
